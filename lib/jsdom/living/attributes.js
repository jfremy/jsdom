...
const idlUtils = require("./generated/util");
const attrGenerated = require("./generated/attributes/Attr");

function attrImpl(attrWrapper) {
  return idlUtils.implForWrapper(attrWrapper, "Attr");
}

function isAttr(obj) {
  return Boolean(obj && attrImpl(obj));
}
...
exports.changeAttribute = function (element, attribute, value) {
  ...
  const impl = attrImpl(target);
  const oldValue = impl._value;
  impl._value = value;

  // Run jsdom hooks; roughly correspond to spec's "An attribute is set and an attribute is changed."
  element._attrModified(impl._name, value, oldValue);
};
...
exports.appendAttribute = function (element, attribute) {
  ...
  if (!reservedNames.has(impl._name)) {
    attributesNNM[impl._name] = attribute;
  }

  // Sync name cache
  const cache = attributesNNM[INTERNAL].attributesByNameMap;
  let entry = cache.get(impl._name);
  if (!entry) {
    entry = [];
    cache.set(impl._name, entry);
  }
  entry.push(attribute);

  // Run jsdom hooks; roughly correspond to spec's "An attribute is set and an attribute is added."
  element._attrModified(impl._name, impl._value, null);
};
...
exports.removeAttribute = function (element, attribute) {
  ...
  for (let i = 0; i < attributeList.length; ++i) {
    if (attributeList[i] === attribute) {
      ...
      // Sync target named properties
      if (!reservedNames.has(impl._name)) {
        delete attributesNNM[impl._name];
      }

      // Sync name cache
      const cache = attributesNNM[INTERNAL].attributesByNameMap;
      const entry = cache.get(impl._name);
      entry.splice(entry.indexOf(attribute), 1);
      if (entry.length === 0) {
        cache.delete(impl._name);
      }

      // Run jsdom hooks; roughly correspond to spec's "An attribute is removed."
      element._attrModified(impl._name, null, impl._value);
...
exports.setAttribute = function (element, attr, namespaceAndLocalNameFlag) {
  ...
  if (impl._element !== null && attr._element !== element) {
    throw new DOMException(DOMException.INUSE_ATTRIBUTE_ERR);
  }

  let oldAttr = null;
  if (namespaceAndLocalNameFlag) {
    oldAttr = exports.getAttributeByNameNS(element, impl._namespace, impl._localName);
  } else {
    oldAttr = exports.getAttributeByName(element, impl._name);
  }
...
exports.setAttributeValue = function (element, localName, value, name, prefix, namespace) {
  ...
  if (attribute === null) {
    const newAttribute = attrGenerated.create([], { namespace, prefix, localName, name, value });
...
exports.copyAttributeList = function (sourceElement, destElement) {
  ...
  for (const sourceAttr of sourceElement._attributes[INTERNAL].attributeList) {
    const sourceImpl = attrImpl(sourceAttr);
